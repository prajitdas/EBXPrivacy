...
public class PrivacyProvider extends ContentProvider {
	private static final String AUTHORITY = "edu.umbc.cs.ebiquity.mithril.ebxprivacy.provider";
	...
	public static void fixFilePermissions() {
		...
		if (files != null)
			for (File file : files)
				if (file.getName().startsWith("edu.umbc.cs.ebiquity.mithril.ebxprivacy.provider.") && file.getName().endsWith(".xml")
	...
	private static void convertRestrictions(Context context) throws IOException {
		File source = new File(Util.getUserDataDirectory(Process.myUid()) + File.separator + "shared_prefs"
				+ File.separator + "edu.umbc.cs.ebiquity.mithril.ebxprivacy.provider.xml");
